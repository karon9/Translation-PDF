...
from translate_pdf.modules.spacy_api import *
...
async def extract_text_coordinates_blocks(pdf_data):
    ...
    """
    # PDFファイルを開く
    document = await asyncio.to_thread(pymupdf.open, stream=pdf_data, filetype="pdf")
...
async def remove_blocks(block_info, token_threshold=10, debug=False, lang="en"):
    ...
    for i in range(len(text_list)):
        ...
        text_list[i] = "".join(
            char for char in text_list[i] if char not in string.punctuation and char not in string.digits
        )
    ...
    if debug:
        ...
        token_Mean = plot_area_distribution(
            areas=tokens,
        ...
        size_Mean = plot_area_distribution(
            areas=sizes,
        ...
        socre_Mean = plot_area_distribution(
            areas=marge_score,
            labels_values=[{"Histogram Low": most_frequent_range[0]}, {"Histogram High": most_frequent_range[1]}],
            title="score Mean",
            xlabel="score",
            ylabel="Frequency",
        )
...
async def remove_textbox_for_pdf(pdf_data, remove_list):
    ...
    """
    doc = await asyncio.to_thread(pymupdf.open, stream=pdf_data, filetype="pdf")
    for remove_data, page in zip(remove_list, doc):
        for remove_item in remove_data:
            rect = pymupdf.Rect(remove_item["coordinates"])  # テキストブロックの領域を取得
...
async def pdf_draw_blocks(
    input_pdf_data, block_info, line_colorRGB=[0, 0, 1], width=5, fill_colorRGB=[0, 0, 1], fill_opacity=1
):
    """
    PDFデータに、デバッグ用、四角い枠を作画します
    """
    doc = await asyncio.to_thread(pymupdf.open, stream=input_pdf_data, filetype="pdf")

    for i, pages in enumerate(block_info):
        page = doc[i]
        for block in pages:
            x0, y0, x1, y1 = block["coordinates"]
            text_rect = pymupdf.Rect(x0, y0, x1, y1)
...
async def preprocess_write_blocks(block_info, to_lang="ja"):
    ...
    # フォント選択
    if to_lang == "en":
        font_path = Path(__file__).parents[1] / "fonts" / "TIMES.TTF"
        a_text = "a"
    elif to_lang == "ja":
        font_path = Path(__file__).parents[1] / "fonts" / "MSMINCHO.TTC"
        a_text = "あ"

    # フォントサイズを逆算+ブロックごとにテキストを分割
    any_blocks = []
    print("Preprocess Write Blocks")
    for page in tqdm(block_info):
        for box in page:
            ...
            while True:
                ...
                # フォントサイズ計算
                font = pymupdf.Font("F0", str(font_path))
        ...
        for block in page_block:
            ...
            while True:
                ...
                result = page.insert_textbox(
                    rect, text, fontsize=font_size, fontname="F0", align=3, lineheight=lh_factor, color=text_color
                )
                if result >= 0:
                    break
                else:
...
async def create_viewing_pdf(base_pdf_path, translated_pdf_path):
    # PDFドキュメントを開く
    doc_base = await asyncio.to_thread(pymupdf.open, stream=base_pdf_path, filetype="pdf")
    doc_translate = await asyncio.to_thread(pymupdf.open, stream=translated_pdf_path, filetype="pdf")
